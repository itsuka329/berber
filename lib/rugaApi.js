...
const insta = async (url) => new Promise((resolve, reject) => {
    axios.get(`${links}/ig?URL=${url}`)
    .then((res) => {
        resolve(`${res.data.getData}`)
    })
    .catch((err) => {
        reject(err)
    })
...
/* API TWITTER IMAGES */
const twitimg = async (url) => new Promise((resolve, reject) => {
    axios.get(`${links}/twimg?URL=${url}`)
    .then((res) => {
...
/* API TWITTER VIDEOS */
const twitvid = async (url) => new Promise((resolve, reject) => {
    axios.get(`${links}/twvid?URL=${url}`)
    .then((res) => {
        resolve(`${res.data.getVideo}`)
    })
    .catch((err) => {
        reject(err)
    })
...
const tiktok = async (url) => new Promise((resolve, reject) => {
    ...
    .then((res) => {
	    const grabData = `${res.data.mp4direct}`
        resolve(grabData)
    })
    .catch((err) => {
        reject(err)
...
/* API TIKTOK INFO */
const tiktokInfo = async (url) => new Promise((resolve, reject) => {
    axios.get(`${links}/tiktok?URL=${url}`)
    .then((res) => {
		const grabInfo = `
✅Sukses Mengunduh Video Tiktok Tanpa Watermark dengan
└Username : *${res.data.nameInfo}*
└Caption : *${res.data.textInfo}*
└Upload Time : *${res.data.timeInfo}*
Download Menggunakan Bot Whatsapp Yaelahdo
`
        resolve(grabInfo)
    })
    .catch((err) => {
...
/* API YOUTUBE */
const ytvid = async (url) => new Promise((resolve, reject) => {
    axios.get(`${links}/ytvid?URL=${url}`)
    .then((res) => {
        resolve(`${res.data.getVideo}`)
    })
    .catch((err) => {
...
/* API YT MUSIC 
const ytmp3info = async (url) => new Promise((resolve, reject) => {
    axios.get(`${links}/ytmus?URL=${url}`)
    .then((res) => {
		const grabMusic = `✅Sukses Mengunduh Audio Youtube dengan\n\n*${res.data.titleInfo}*\n\n*${res.data.sizeInfo}*`
        resolve(grabMusic)
    })
    .catch((err) => {
        reject(err)
