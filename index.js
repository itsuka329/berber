...
const start = (pakforlay = new Client()) => {
    ...
    console.log(color(figlet.textSync('Otsuka BOT', { font: 'Ghost', horizontalLayout: 'default' })))
    console.log(color(figlet.textSync('----------------', { horizontalLayout: 'default' })))
    console.log(color('[DEV]'), color('Otsuka', 'yellow'))
	...
	// kondisi ketika batas member group belum tercapai, ubah di file settings/setting.json
        ...
        await pakforlay.simulateTyping(chat.id, true).then(async () => {
          await pakforlay.sendText(chat.id, `Hai minna~, Im Otsuka BOT. To find out the commands on this bot type ${prefix}menu`)
        })
	    }
	}
    })
